...
public class DatabaseHandler extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase db) {
        String create_sv_table = String.format("CREATE TABLE %s(%s INTEGER PRIMARY KEY, %s TEXT, %s FLOAT);", TABLE_SV, KEY_ID, KEY_NAME, KEY_DIEMTB);
    ...
    public SinhVien getSV(int id) {
        ...
        if(cursor != null && cursor.moveToFirst()) {
            sv = new SinhVien(cursor.getInt(0), cursor.getString(1), cursor.getFloat(3));
            cursor.close();
        }
        db.close();
        return sv;
    ...
    public List<SinhVien> getAllSV() {
        ...
        if (cursor != null && cursor.moveToFirst()) {
            ...
            do {
                ...
                float diem = cursor.getFloat(diemIndex);
                SinhVien sv = new SinhVien(id, name, diem);
                listSV.add(sv);
